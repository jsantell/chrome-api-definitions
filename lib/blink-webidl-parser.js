...
function cleanWebIDL (string) {
  ...
  function fixUnquotedEnums(chunk) {
    ...
    // Check again to make sure we're still in enum. Abort
    // for cases of entire enum on single line, and
    // in a list, where the } is on the same line as the last value.
    if (/\};/.test(chunk)) {
      IN_ENUM = false;
    }
