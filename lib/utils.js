...
var NS_MAP = require("../namespace-map.json");
...
function getDefinitionPath (name, root) {
  ...
  // There are a few namespace maps that are inconsistent -- record them
  // in `ROOT/namespace-map.json` and just check here.
  if (NS_MAP[name]) {
    nsPath = NS_MAP[name].split("/");
  } else {
    nsPath = name.split(".").map(underscoreify);
  }
  ...
  // If we're here, try again but instead of converting namespaces
  // to directories like:
  // "devtools.inspectedWindow" -> "devtools/inspected_window"
  // convert them to just underscored separators:
  // "system.cpu" -> "system_cpu"
  //
  // Yes both of these formats are in the Chromium source code.
  if (nsPath.length > 1) {
    return getDefinitionPath(nsPath.join("_"), root);
  }
}
...
function attachMeta (definition, namespace, root) {
  ...
  catch(e) { console.log(e, namespace) }
